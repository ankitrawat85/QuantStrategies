import numpy as np
import pandas as pd
from MQF.MQF.QuantStrategies.project.time_varying import TimeVaryingPortfolioStrategy
from MQF.MQF.QuantStrategies.project.trade_rule import SIGN,TREND
#from data import trade_rule
...
class CorrAdjustedTSMOMStrategy(TimeVaryingPortfolioStrategy):
    def __init__(self,
        ...
        super().__init__( data, sigma_target, lookback_vol, volatilty_estimator, include_transaction)
    ...
    def pre_strategy(self):
        ...
        if str.lower(self.trade_rule_name) == 'sign':
            rule = SIGN(self.aggregated_assets, self.daily_ret, 252)
        elif str.lower(self.trade_rule_name) == 'trend':
            rule = TREND(self.aggregated_assets, self.daily_ret, self.lookback_trend)
        else:
