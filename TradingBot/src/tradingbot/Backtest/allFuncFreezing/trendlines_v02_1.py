...
if MODE == 'zerodha':
    ...
    DF_FULL = fetch_and_prepare_zerodha(ROOT, FROM, TO, TOKEN)
elif MODE == 'csv':
    DF_FULL = load_csv(CSV_PATH, ts_col=TS_COL)
else:
    DF_FULL = make_dummy_ohlc(n=500, start='2023-01-01', freq='B')
...
if __name__ == "__main__":
    ...
    # 3) Or smoke test with dummy data
    sys.path.insert(0, ROOT); os.chdir(ROOT)
    DF_FULL = fetch_and_prepare_zerodha(ROOT, FROM, TO, TOKEN)
    #test_df = make_dummy_ohlc(DF_FULL[:-500])
    segs2, sigs2 = run_signal_scan(DF_FULL[-900:])
    print(signals_to_dataframe(sigs2).tail())
    #result = summarize_as_of_last_close(DF_FULL)
    #print(result["signal"], result["slope_sup"], result["slope_res"], result["pattern"])
