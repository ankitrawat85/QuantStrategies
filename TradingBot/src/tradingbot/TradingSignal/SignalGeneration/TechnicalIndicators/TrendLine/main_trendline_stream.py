...
def stream(args,df_in=None):
    if isinstance(df_in, pd.DataFrame):
        df = df_in.copy()
    elif getattr(args, "csv", ""):
        df = pd.read_csv(args.csv)
    else:
        raise ValueError("No input data: pass --csv or df_in DataFrame")
    ...
    # only write if explicitly enabled
    if getattr(args, "write_csv", False) and args.events:
        ev_df.to_csv(args.events, index=False)
    if getattr(args, "write_csv", False) and args.points:
        pt_df.to_csv(args.points, index=False)
    ...
    if getattr(args, "return_results", False):
        return {"events": ev_df, "points": pt_df, "snapshot": snapshot_out_df}
    return 0
...
def build_parser(defaults=None):
    ...
    # IO
    p.add_argument("--csv", default="", help="Prices CSV (date, open, high, low, close)")
    p.add_argument("--events", default="", help="Output events CSV")
    p.add_argument("--points", default="", help="Output points CSV")
    # Windows
    ...
    p.add_argument("--write-csv", action="store_true", help="Write events/points/snapshot CSVs when set")
    p.add_argument("--return-results", action="store_true", help="Return in-memory results dict instead of exit code")

    # Snapshot controls
    p.add_argument("--snapshot-date", default="", help="YYYY-MM-DD; if empty uses last bar")
    p.add_argument("--snapshot-out", default="", help="Write CSV snapshot (only with --write-csv)")
