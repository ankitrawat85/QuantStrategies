...
class Vol:
        ...
        for i in range(0, len(opt)):
            if opt.iloc[i]['Option Type'] == 'CE':
                opt.iloc[i, opt.columns.get_loc('impliedvolatility')] = mibian.BS([opt.iloc[i]['futures_price'],
                                                                   ...
                                                                   callPrice=opt.iloc[i]['Settle Price']
                                                                   ).impliedVolatility
            else:
                opt.iloc[i, opt.columns.get_loc('impliedvolatility')] = mibian.BS([opt.iloc[i]['futures_price'],
                                                                    opt.iloc[i]['Strike Price'],
                                                                    0,
                                                                    opt.iloc[i]['time_diff']],
                                                                   putPrice=opt.iloc[i]['Settle Price']
                                                                   ).impliedVolatility
        return opt
    ...
    def impliedPutVol(self,model):
        ...
        if (model == "BlackScholes"):
            impliedVolput = op.brentq(lambda x: self.price -OptionPricing().BlackScholesPut(self.spotprice, self.strike, self.riskfreerate, x, self.timeperiod),1e-6, 1)
        return impliedVolput

    def impliedCallVol(self, model):
        if (model == "BlackScholes"):
            impliedVolcall = op.brentq(lambda x: self.price -
                                          OptionPricing().BlackScholesCall(self.spotprice, self.strike, self.riskfreerate, x, self.timeperiod),
...
put_df_option = put_df_option[["Date","Expiry","Option Type","Settle Price","Strike Price","Underlying Value","time_diff","futures_price"]]
call_df_option = call_df_option[["Date","Expiry","Option Type","Settle Price","Strike Price","Underlying Value","time_diff","futures_price"]]
...
## Through Library
"""
call_df_option = Vol(spotprice =call_df_option["Settle Price"],Strike=call_df_option["Strike Price"], riskfreerate=0.05, price=put_df_option["Underlying Value"], T=put_df_option["time_diff"]).implied_volatility_options(call_df_option)
print(call_df_option)
put_df_option = Vol(spotprice =call_df_option["Settle Price"],Strike=call_df_option["Strike Price"], riskfreerate=0.05, price=put_df_option["Underlying Value"], T=put_df_option["time_diff"]).implied_volatility_options(put_df_option)
print(put_df_option)
...
## thorugh my code
call_df_option["impliedvolatility"] = call_df_option.apply(lambda x : Vol(spotprice =x["futures_price"],Strike=x["Strike Price"], riskfreerate=0, price=x["Settle Price"], T=x["time_diff"]).impliedCallVol(model="BlackScholes"),axis =1)

put_df_option["impliedvolatility"] = put_df_option.apply(lambda x : Vol(spotprice =x["futures_price"],Strike=x["Strike Price"], riskfreerate=0, price=x["Settle Price"], T=x["time_diff"]).impliedPutVol(model="BlackScholes"),axis =1)
df = pd.concat([call_df_option,put_df_option])
...
#print(df[df["Date"] == "2021-01-11"][["Strike Price","impliedvolatility","sabrsigma"]])
df_plot = df[df["Date"] == "2021-01-10"][["Strike Price","impliedvolatility","sabrsigma"]].sort_values(by = "Strike Price")
print(df_plot)
df_plot.to_csv("sabrsigma.csv")
