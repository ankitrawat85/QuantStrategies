...
def Place2LeggedOrder(Trade1L, orderqty1, orderqty2, orderprice1, orderprice2, nearestexpfuttrdsymbol1, nearestexpfuttrdsymbol2, isentrytrade, pairnum, current_signal, exitreason, strategy_state):
    try:
        ...
        if (abs(orderqty1) > 0) :
            ...
            orderid1 = zerodha.place_order(api_key, access_token, fo_exchange, 'FNO', trdsymbol1, Trade1L, "LIMIT", orderqty1, orderprice1, product, order_duration, order_variety='REGULAR', printerrormessage = True)
            if (orderid1 == None):
                # print("Error occured in placing order for " + spotsymbol + ". Algo stopped by system.")
                # stopalgo = True
                # break
                print(Trade1L + " order for " + trdsymbol1 + " could not be placed.")
                # VERY IMPORTANT TODO
                # check if last order on this leg failed, then place the order on this leg again
                rtn =  False
                
            if (orderid1 != ''):
                print("Order placed successfully. " + Trade1L + " order for " + trdsymbol1 + " qty:" + str(orderqty1) + " @ " + str(orderprice1) + " orderid: " + orderid1)
        ...
        if (abs(orderqty2) > 0) :
            ...
            # API Call to place the order
            orderid2 = zerodha.place_order(api_key, access_token, fo_exchange, 'FNO', trdsymbol2, Trade2L, "LIMIT", abs(orderqty2), orderprice2, product, order_duration, order_variety='REGULAR', printerrormessage = True)
            if (orderid2 == None):
                # print("Error occured in placing order for " + spotsymbol + ". Algo stopped by system.")
                # stopalgo = True
                # break
                print(Trade2L + " order for " + trdsymbol2 + " could not be placed.")
                # VERY IMPORTANT TODO
                # check if last order on this leg failed, then place the order on this leg again
                rtn =  False
            
            if (orderid2 != ''):
                print("Order placed successfully. " + Trade2L + " order for " + trdsymbol2 + " qty:" + str(orderqty2) + " @ " + str(orderprice2) + " orderid: " + orderid2)
                ...
                if (is_order_executing == False):
                    ...
                    if ((current_signal != 0) or (current_pos_leg1 != 0) or (current_pos_leg2 != 0)):
                        ...
                        if ((orderqty1 > 0) or (-orderqty2 > 0)) :
                            ...
                            if not Place2LeggedOrder("BUY", orderqty1, orderqty2, orderprice1, orderprice2, nearestexpfuttrdsymbol1, nearestexpfuttrdsymbol2, isentrytrade, pairnum, current_signal, exitreason, strategy_state):
                                errctr = errctr + 1
                                if (errctr > maxerrcnt):
                                    stopalgo = True
                            ...
                            if not Place2LeggedOrder("SELL", orderqty1, orderqty2, orderprice1, orderprice2, nearestexpfuttrdsymbol1, nearestexpfuttrdsymbol2, isentrytrade, pairnum, current_signal, exitreason, strategy_state):
                                errctr = errctr + 1
                                if (errctr > maxerrcnt):
                                    stopalgo = True
