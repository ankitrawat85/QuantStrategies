...
def backtest(parameters, price1, price2, init_capital, max_capital_deploy, buy_margin, 
             sell_margin, transactioncost_rate, slippage_rate, fixedcapital = False):
        ...
        for i in range(len(signal_score)):
            ...
            if (pos == 0):
                ...
                if (signal_score[i] < short_exit):
                    ...
                    gross_pnl = qty1 * (price1[i] - entry_price1) + qty2 * (price2[i] - entry_price2)
                    
                    gross_rtn = gross_pnl / margin_blocked
                    
                    net_rtn = gross_rtn - (transactioncost_rate + slippage_rate)
                    
                    net_pnl = net_rtn * margin_blocked
                    
                    trade_pnl = np.append(trade_pnl, net_pnl)
                    mtm_pnl = np.append(mtm_pnl, net_pnl)
